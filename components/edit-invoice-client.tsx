...
export default function EditInvoiceClient({ invoice }: EditInvoiceClientProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [selectedPatient, setSelectedPatient] = useState<Patient | null>(invoice.patient);
  ...
  const [discountType, setDiscountType] = useState<'percentage' | 'fixed'>(
    invoice.discountType as 'percentage' | 'fixed'
  );
  const [branch, setBranch] = useState<string>(invoice.branch);
  const [corporateId, setCorporateId] = useState<string>(invoice.corporateId || '');
  ...
  const initializeInvoiceItems = useCallback(() => {
    ...
    // Initialize search terms with existing product names
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedPatient) {
      setError('Please select a patient before updating the invoice.');
      setLoading(false);
      return;
    }
    ...
    <div className="bg-white shadow rounded-lg">
      <div className="px-4 py-5 sm:px-6">
        ...
        <p className="mt-1 max-w-2xl text-sm text-gray-500">
          Update the invoice details, products, and pricing information.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="border-t border-gray-200">
        <div className="px-4 py-5 sm:p-6">
          ...
          <div className="mb-6">
            ...
            {patientSelectionMode === 'none' && !selectedPatient && (
        ...
        <div className="bg-gray-50 px-4 py-3 text-right sm:px-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="rounded-md bg-orange-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-orange-500 disabled:opacity-50"
          >
